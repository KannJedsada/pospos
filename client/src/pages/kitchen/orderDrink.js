  ...
  const fetchOrderDetail = async () => {
    try {
      ...
      const filteredOrders = res.data.data.filter(
        (order) => (order.dish_status === 1 || order.dish_status === 2) && order.typename === "เครื่องดื่ม"
      );
  ...
  const updateMenuStatus = async (menuName, newStatus) => {
    try {
      ...
      if (ordersToUpdate.length === 0) {
        Swal.fire("Error", "ไม่มีรายการอาหาร", "error");
        return;
      }
